...
<body style="padding:0 20px;  min-width:1200px;">
	...
	<form action="#"  method="post" style="position:relative; left:10%; width:1035px;" id="myform">
	   ...
	   <input id="tit" type="text" style="width: 560px;height: 20px; margin-bottom: 6px;" maxlength="100"  name="title"/><span style="color:red" id="errorTip"></span>
	   <br /><br />
	   ...
	   <label class="mylabel" style="color:#FFB90F;"><span>文章正文</span></label>
		...
		<s:iterator value="#request.text_type" var="tt">
			...
			<%-- <label for="che1"> <span class="myfont_kai_B" >人工智能</span> </label><input type="radio" value="AI" name="che" id="che1"  checked/>
			<label for="che2"> <span class="myfont_kai_B">算法</span> </label><input type="radio" value="ALGORITHM" name="che" id="che2"/> --%>
			
			<s:if test="#tt.texttypeId==1">
				<label for="che1"> <span class="myfont_kai_B" >  <s:property value="#tt.ttName" />  </span> </label>
				<input type="radio" value="<s:property value="#tt.ttfName" />" name="type" id="che1"  checked/>
			</s:if>
	    ...
	    <button id="sub_but" type="button" onclick="setVal()" class="mysub" style="position:relative;left:-10%;">
	    	<span class="myfont_kai_B">发表文章</span>
	    </button>
    ...
    <script type="text/javascript">
    	...
    	function setVal() {
    		...
    		if(value == "") {
	    		...
	    		//$("#txt").text(txt);
	    		var url = "Jsecond/MyJBlog_saveBlogText";
	    		var title=$("#tit").val();
	    		...
	    		var texttype=$("input[name='type']:checked").val();
					...
					$.post(url, Myjson, function(data) {
						...
						window.location.href="second/MyBlog_displayText?blogid="+result;
		    		});
	    		}
	    		//$("#myform").submit();
    	}
